...
여러 과목의 PDF를 바탕으로 만든 시험 대비용 정적 웹사이트입니다. 과목별 원본 콘텐츠와 생성 결과를 분리해서, 강의 PDF가 계속 늘어나도 관리하기 쉽게 구성했습니다.
...
`study-data.js`는 생성 파일이므로 직접 수정하지 말고 `content/courses/<course-id>/site.json`, `content/courses/<course-id>/lectures/*.json`을 수정한 뒤 다시 빌드하면 됩니다.

로컬 PDF 폴더 경로는 빌드에만 쓰이며, 생성된 `study-data.js`에는 포함되지 않도록 처리되어 있습니다.
...
- `scripts/build-study-data.py`: 모든 과목 PDF 폴더 스캔 + `study-data.js` 생성
...
7. 사이트에 `Draft` 카드가 남아 있지 않은지 확인합니다.
